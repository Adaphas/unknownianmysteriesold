...
import net.mcreator.unknownianmysteries.item.CyanSeaItem;
...
public class CyanSeaDimension extends UnknownianmysteriesModElements.ModElement {
	@ObjectHolder("unknownianmysteries:cyan_sea_portal")
	public static final CustomPortalBlock portal = null;

	public CyanSeaDimension(UnknownianmysteriesModElements instance) {
		super(instance, 89);
	...
	public void init(FMLCommonSetupEvent event) {
		...
		replaceableBlocks.add(Blocks.CYAN_TERRACOTTA);
		replaceableBlocks.add(ForgeRegistries.BIOMES.getValue(new ResourceLocation("unknownianmysteries:cyan_cold_ocean")).getGenerationSettings()
				.getSurfaceBuilder().get().getConfig().getTop().getBlock());
		replaceableBlocks.add(ForgeRegistries.BIOMES.getValue(new ResourceLocation("unknownianmysteries:cyan_cold_ocean")).getGenerationSettings()
				.getSurfaceBuilder().get().getConfig().getUnder().getBlock());
		replaceableBlocks.add(ForgeRegistries.BIOMES.getValue(new ResourceLocation("unknownianmysteries:cyan_warm_ocean")).getGenerationSettings()
				.getSurfaceBuilder().get().getConfig().getTop().getBlock());
		replaceableBlocks.add(ForgeRegistries.BIOMES.getValue(new ResourceLocation("unknownianmysteries:cyan_warm_ocean")).getGenerationSettings()
				.getSurfaceBuilder().get().getConfig().getUnder().getBlock());
	...
	public void clientLoad(FMLClientSetupEvent event) {
		...
		DeferredWorkQueue.runLater(() -> {
			try {
				...
				effectsRegistry.put(new ResourceLocation("unknownianmysteries:cyan_sea"), customEffect);
			} catch (Exception e) {
				e.printStackTrace();
			}
		});
	...
	public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("cyan_sea_portal", Vector3i::compareTo, 300);
	...
	public static class POIRegisterHandler {
		...
		public void registerPointOfInterest(RegistryEvent.Register<PointOfInterestType> event) {
			poi = new PointOfInterestType("cyan_sea_portal", Sets.newHashSet(ImmutableSet.copyOf(portal.getStateContainer().getValidStates())), 0, 1)
					.setRegistryName("cyan_sea_portal");
	...
	public void initElements() {
		...
		elements.items.add(() -> new CyanSeaItem().setRegistryName("cyan_sea"));
	}
			...
			if (!entity.isPassenger() && !entity.isBeingRidden() && entity.isNonBoss() && !entity.world.isRemote && true) {
				if (entity.func_242280_ah()) {
					entity.func_242279_ag();
						...
						new ResourceLocation("unknownianmysteries:cyan_sea"))) {
					entity.func_242279_ag();
					teleportToDimension(entity, pos,
							RegistryKey.getOrCreateKey(Registry.WORLD_KEY, new ResourceLocation("unknownianmysteries:cyan_sea")));
				} else {
					entity.func_242279_ag();
