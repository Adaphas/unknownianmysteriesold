...
package net.mcreator.unknownianmysteries.world.dimension;
...
import net.mcreator.unknownianmysteries.procedures.BlueMoonGravityStopProcedure;
import net.mcreator.unknownianmysteries.procedures.BlueMoonGravityStartProcedure;
import net.mcreator.unknownianmysteries.item.BlueMoonItem;
import net.mcreator.unknownianmysteries.block.UnknownianRedPortalBlockBlock;
...
public class BlueMoonDimension extends UnknownianmysteriesModElements.ModElement {
	@ObjectHolder("unknownianmysteries:blue_moon_portal")
	public static final CustomPortalBlock portal = null;

	public BlueMoonDimension(UnknownianmysteriesModElements instance) {
		super(instance, 95);
	...
	public void init(FMLCommonSetupEvent event) {
		...
		replaceableBlocks.add(BlueMoonRockBlock.block);
		replaceableBlocks.add(ForgeRegistries.BIOMES.getValue(new ResourceLocation("unknownianmysteries:blue_moon_lowlands")).getGenerationSettings()
				.getSurfaceBuilder().get().getConfig().getTop().getBlock());
		replaceableBlocks.add(ForgeRegistries.BIOMES.getValue(new ResourceLocation("unknownianmysteries:blue_moon_lowlands")).getGenerationSettings()
				.getSurfaceBuilder().get().getConfig().getUnder().getBlock());
		replaceableBlocks.add(ForgeRegistries.BIOMES.getValue(new ResourceLocation("unknownianmysteries:blue_moon_flatlands")).getGenerationSettings()
				.getSurfaceBuilder().get().getConfig().getTop().getBlock());
		replaceableBlocks.add(ForgeRegistries.BIOMES.getValue(new ResourceLocation("unknownianmysteries:blue_moon_flatlands")).getGenerationSettings()
				.getSurfaceBuilder().get().getConfig().getUnder().getBlock());
		replaceableBlocks.add(ForgeRegistries.BIOMES.getValue(new ResourceLocation("unknownianmysteries:blue_moon_highlands")).getGenerationSettings()
				.getSurfaceBuilder().get().getConfig().getTop().getBlock());
		replaceableBlocks.add(ForgeRegistries.BIOMES.getValue(new ResourceLocation("unknownianmysteries:blue_moon_highlands")).getGenerationSettings()
				.getSurfaceBuilder().get().getConfig().getUnder().getBlock());
	...
	public void clientLoad(FMLClientSetupEvent event) {
		...
		DeferredWorkQueue.runLater(() -> {
			try {
				...
				effectsRegistry.put(new ResourceLocation("unknownianmysteries:blue_moon"), customEffect);
			} catch (Exception e) {
				e.printStackTrace();
			}
		});
	...
	public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("blue_moon_portal", Vector3i::compareTo, 300);
	...
	public static class POIRegisterHandler {
		...
		public void registerPointOfInterest(RegistryEvent.Register<PointOfInterestType> event) {
			poi = new PointOfInterestType("blue_moon_portal", Sets.newHashSet(ImmutableSet.copyOf(portal.getStateContainer().getValidStates())), 0, 1)
					.setRegistryName("blue_moon_portal");
	...
	public void initElements() {
		...
		elements.items.add(() -> new BlueMoonItem().setRegistryName("blue_moon"));
	}
			...
			if (!entity.isPassenger() && !entity.isBeingRidden() && entity.isNonBoss() && !entity.world.isRemote && true) {
				if (entity.func_242280_ah()) {
					...
					teleportToDimension(entity, pos,
							RegistryKey.getOrCreateKey(Registry.WORLD_KEY, new ResourceLocation("unknownianmysteries:blue_moon")));
				} else {
					entity.func_242279_ag();
	...
	public static class CustomPortalSize {
		private static final AbstractBlock.IPositionPredicate POSITION_PREDICATE = (state, blockReader, pos) -> {
			return state.getBlock() == UnknownianRedPortalBlockBlock.block;
		};
				...
				for (int l1 = -1; l1 < 2; ++l1) {
					for (int k2 = 0; k2 < 2; ++k2) {
						for (int i3 = -1; i3 < 3; ++i3) {
							BlockState blockstate1 = i3 < 0 ? UnknownianRedPortalBlockBlock.block.getDefaultState() : Blocks.AIR.getDefaultState();
			...
			for (int k1 = -1; k1 < 3; ++k1) {
				for (int i2 = -1; i2 < 4; ++i2) {
					if (k1 == -1 || k1 == 2 || i2 == -1 || i2 == 3) {
						...
						this.world.setBlockState(blockpos$mutable, UnknownianRedPortalBlockBlock.block.getDefaultState(), 3);
					}
				}
			}
	...
	public void onPlayerChangedDimensionEvent(PlayerEvent.PlayerChangedDimensionEvent event) {
		...
		if (event.getFrom() == RegistryKey.getOrCreateKey(Registry.WORLD_KEY, new ResourceLocation("unknownianmysteries:blue_moon"))) {

			BlueMoonGravityStopProcedure.executeProcedure(Stream.of(new AbstractMap.SimpleEntry<>("entity", entity)).collect(HashMap::new,
					(_m, _e) -> _m.put(_e.getKey(), _e.getValue()), Map::putAll));
		}
		if (event.getTo() == RegistryKey.getOrCreateKey(Registry.WORLD_KEY, new ResourceLocation("unknownianmysteries:blue_moon"))) {

			BlueMoonGravityStartProcedure.executeProcedure(Stream.of(new AbstractMap.SimpleEntry<>("entity", entity)).collect(HashMap::new,
					(_m, _e) -> _m.put(_e.getKey(), _e.getValue()), Map::putAll));
		}
	}
}
