...
public class BlueMoonHighlandsBiome extends UnknownianmysteriesModElements.ModElement {
	public static Biome biome;

	public BlueMoonHighlandsBiome(UnknownianmysteriesModElements instance) {
		super(instance, 90);
	...
	private static class BiomeRegisterHandler {
		...
		public void registerBiomes(RegistryEvent.Register<Biome> event) {
			if (biome == null) {
				...
				biome = new Biome.Builder().precipitation(Biome.RainType.NONE).category(Biome.Category.NONE).depth(0.7f).scale(0.7999999999999999f)
						.temperature(0.1f).downfall(0f).setEffects(effects).withMobSpawnSettings(mobSpawnInfo.copy())
						.withGenerationSettings(biomeGenerationSettings.build()).build();
				event.getRegistry().register(biome.setRegistryName("unknownianmysteries:blue_moon_highlands"));
			}
		}
	}
